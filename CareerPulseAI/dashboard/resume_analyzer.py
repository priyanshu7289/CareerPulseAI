...
def render():
    ...
    if uploaded_file is not None:
        ...
        c3.metric("Skills found", f"{len(result['matched_skills'])} / {len(result['required_skills'])}")
        ...
        if result["missing_skills"]:
            ...
            st.dataframe(
                [{"Skill": r["skill"], "Course": r["name"], "Platform": r["platform"],
                  "Cost": "Free" if r["is_free"] else "Paid", "Duration": f"{r['weeks']} wks",
                  "Priority": r["priority"]} for r in recs],
                use_container_width=True, hide_index=True,
            )

            path = suggest_career_path(target_role, result["missing_skills"])
            st.info(f"**Next career step:** {path['next_career_step']}\n\n"
                     f"**Suggested project:** {path['suggested_projects'][0]}")

        # Salary estimate
        model = get_model()
        salary_result = predict_salary(model, years_exp, "Bangalore", "Python + ML", "B.Tech", target_role)
        ...
        if st.button("📥 Generate full PDF career report", type="primary"):
            with st.spinner("Generating your personalized PDF report..."):
                ...
                trending = [("Python", 84), ("SQL", 76), ("Power BI", 62), ("Generative AI", 58)]
                generate_career_report(
                    ...
                    recommendations=generate_recommendations(result["missing_skills"]) or [
                        {"skill": "—", "name": "Keep building on your strengths", "platform": "—", "is_free": True, "weeks": 0, "priority": "Low"}
                    ],
                    trending_skills=trending,
                )
            with open(output_path, "rb") as f:
                st.download_button("Download career report (PDF)", f, file_name="CareerPulse_Report.pdf", mime="application/pdf")
    else:
